    ...
    FString& BreakReason, FString& NextScenarioName, FString& ErrorMessage)
{
    ...
    Status = TryExecuteAutoSave(Args, ProcessName, Widget, ErrorMessage)
        ? EShidenProcessStatus::Next
        : EShidenProcessStatus::Error;
}

bool UShidenAutoSaveCommand::IsWidgetInitialized(const UShidenWidget* Widget)
{
    const FVector2d DesiredSize = Widget->GetDesiredSize();
    return DesiredSize.X > 0 && DesiredSize.Y > 0;
}

bool UShidenAutoSaveCommand::TryExecuteAutoSave(const FAutoSaveCommandArgs& Args, const FString& ProcessName, UShidenWidget* Widget, FString& ErrorMessage)
{
    ...
    {
        Widget->SaveGameWithScreenCapture(Args.SlotName);
    }
    else if (Args.OverwriteThumbnail == TEXT("None"))
    {
        Widget->SaveGame(Args.SlotName, Cast<UTexture2D>(FSlateNoResource().GetResourceObject()));
    }
    else
    {
        ...
        Widget->SaveGame(Args.SlotName, Cast<UTexture2D>(Thumbnail));
    }
